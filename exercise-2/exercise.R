...
for(i in 1:100) {
  ...
  training.sample <- sample(seq_len(nrow(homes)), size = sample.size)
  training.data <- homes[training.sample,]
  test.data <- homes[-training.sample,]
  ...
  assess <- AssessFit(basic.fit, data = test.data)
